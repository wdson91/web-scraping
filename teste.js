...
const fs = require('fs');
const url = 'https://www.linkedin.com/jobs/search/?keywords=Desenvolvedor%20Junior%20Javascript&location=Brasil&locationId=&geoId=106057199&f_TPR=r604800&f_WT=2%2C1%2C3&position=1&pageNum=0';
const url2 = "https://www.linkedin.com/jobs/search?keywords=Industria&location=Portugal&locationId=&geoId=100364837&f_TPR=r604800&position=1&pageNum=0";
...
(async() => {
    ...
    setTimeout(async() => {
        ...
        fs.writeFile('vagasIndustriaPortugal.txt', JSON.stringify(data, null, 2), (err) => {
...
async function scrapeData(page) {
    ...
    const titles = await page.$$eval('.base-search-card__title', (titles) => titles.map((title) => title.innerText));
    const companies = await page.$$eval('.base-search-card__subtitle', (companies) =>
        companies.map((company) => company.innerText)
    );
    const locations = await page.$$eval('.job-search-card__location', (locations) =>
        locations.map((location) => location.innerText)
    );
